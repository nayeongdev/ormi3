...
// 상품 카드 만드는 부분
// 상품 이미지 주소, 가격, 상품명을 받으면 상품 카드를 만들어주는 함수
function createProductCard(imgUrl, name, price, onClick) {
  ...
  // $productCard.append($thumbnailImg, $productName, $productPrice);
...
function main() {
  ...
  fetch(BASE_URL + 'mall').then((res) => {
    ...
    json.forEach(data => {
      ...
      $productCard = createProductCard(productImgUrl, productName, productPrice, onClick);
